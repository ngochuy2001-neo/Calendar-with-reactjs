...
const CalendarDays = ({
  ...
  const checkExisted = (
    dayArray: dayReturnFormat[],
    compareDay: dayReturnFormat
  ) => {
  ...
  return (
    <div className="flex justify-around flex-col">
      <ul className="grid grid-cols-7">
        <li className="w-[70px] flex justify-center items-center h-[50px]">
          Sun
        </li>
        <li className="w-[70px] flex justify-center items-center h-[50px]">
          Mon
      ...
      <ul className="grid grid-cols-7">
        {renderMonth?.map((value, index) => {
          const state = checkActive(
            currentMonth,
            { date: value.date, month: value.month, year: value.year },
            dayStorage
          );
          return (
            <Days
