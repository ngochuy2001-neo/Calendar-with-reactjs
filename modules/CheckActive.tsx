...
const checkActive = (currentMonth: number, dateCheck: dateFormat, dayStorage: dateFormat[]) =>{
  if (dayStorage.length == 1){
    ...
    let beforeDate = new Date(dayStorage[0].year, dayStorage[0].month, dayStorage[0].date)
    let afterDate = new Date(dayStorage[1].year, dayStorage[1].month, dayStorage[1].date)
    if(beforeDate > afterDate) {
      const temp = afterDate;
      afterDate = beforeDate;
      beforeDate = temp;
    };
    const currentCheckDate = new Date(dateCheck.year, dateCheck.month, dateCheck.date);
    const isInTimelime = currentCheckDate >= beforeDate && currentCheckDate <= afterDate; 
    if(isInTimelime) {
      return "active";
    }
  }
